...
if [[ -z "$TMUX" ]] && [[ "$TERM" != "screen" ]]; then
  if ! tmux has-session -t "${_TMUX_SESSION}" 2>/dev/null; then
    if tmux new-session -d -s "${_TMUX_SESSION}" 2>/dev/null; then
  ...
  tmux attach-session -t "${_TMUX_SESSION}" 2>/dev/null || {
    sleep 1
    tmux attach-session -t "${_TMUX_SESSION}" 2>/dev/null || true
  }
fi
unset _TMUX_SESSION
