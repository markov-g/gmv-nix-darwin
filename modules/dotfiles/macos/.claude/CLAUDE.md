...
This file loads at the start of every Claude Code session, in every repo.
It encodes how I work, regardless of project. Project-specific rules
(build commands, framework choices, repo layout) live in `./CLAUDE.md`
at each repo root, not here.

Iterate on this file. When Claude does something I disagree with, ask
...
Note: this file is reinforcement, not enforcement. Real enforcement lives
in `~/.claude/settings.json` (permissions allowlist) and hooks
(`PreToolUse` blocking). Treat both layers as load-bearing.
...
the user's task. Subagent configs live in `~/.claude/agents/` (user) or
`.claude/agents/` (project). When in doubt: more compute via subagents
beats overloading the main thread.
...
Primary languages: Python, Rust, Swift, .NET. The user file stays stack-
agnostic on purpose; per-stack and per-project conventions belong in
each repo's `./CLAUDE.md`. Cross-cutting defaults:
...
- Use `#` during a session to capture a rule I want persisted. Pick the
  right scope (user vs project) deliberately.
- If two CLAUDE.md files conflict, surface the conflict. Do not pick
  silently.
- For long, structured procedures, prefer a Skill or `.claude/rules/`
  file over bloating CLAUDE.md.
...
Claude notices the function uses camelCase while the rest of the file
